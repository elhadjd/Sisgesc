...
const getByMonth = ((date)=>{
    return Request('getRelatorProductsByMonth',date)
})
...
const Request = ( (rota,param = null)=>{
    ...
    .then((response) => {
        emits('progres')
        emits('RelatorByMonth',response.data,Data.Active.date.inicio,Data.Active.date.fin,Data.Active.month)
    }).catch((err) => {
        emits('progres')
        console.log(err);
    });
...
const RelatorBayDay = ((day)=>{
    if (Data.Active.date.fin == null && Data.Active.date.inicio != null) {
        ...
        return Request('IntervalDateInventory',`${Data.Active.month}/${Data.Active.year}/${Data.Active.date.inicio}/${Data.Active.date.fin}`)
    }
...
@import '../../../assets/stock/scss/calendar';
</style>
